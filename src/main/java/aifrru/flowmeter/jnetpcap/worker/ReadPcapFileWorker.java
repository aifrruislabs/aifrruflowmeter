...
import static aifrru.flowmeter.jnetpcap.Utils.*;
...
public class ReadPcapFileWorker extends SwingWorker<List<String>,String> {
    ...
    protected List<String> doInBackground() {
        ...
        if (pcapPath.isDirectory()) {
            ...
            if (!isPcapFile(pcapPath)) {
                ...
                publish("AifrruFlowMeter received 1 pcap file");
                publish("");
                publish("");
    ...
    private void readPcapDir(File inputPath, String outPath) {
        ...
        logger.debug("AifrruFlowMeter found :{} pcap files", file_cnt);
        publish(String.format("AifrruFlowMeter found :%s pcap files", file_cnt));
        publish("");
        publish("");
